...
def render_dashboard():
    ...
    due_accent = "metric-card-due" if today_due > 0 else ""; done_accent = "metric-card-done" if completed_today > 0 else ""; streak_accent = "metric-card-streak" if streak_count > 0 else ""
...
def render_add_task():
    ...
    with tab1:
        st.subheader("New Assignment / Study Goal"); st.caption("Schedule a study session or coursework block. Auto-schedule lets the AI place it optimally.")
    ...
    with tab3:
        st.subheader("Exam Entry"); st.caption("Log an upcoming exam. High priority, appears in red.")
        ...
        if engine_mode == "Compare Both":
            ...
            with st.expander("Workload Balance Comparison"):
                for label, res in [("Greedy", st.session_state.greedy_res), ("CP-SAT", st.session_state.cpsat_res)]:
                    ...
                    if daily: st.markdown(f"**{label} — Hours per Day:**"); st.bar_chart(pd.DataFrame({"Day": list(daily.keys()), "Hours": list(daily.values())}).set_index("Day"), color="#8430CE" if label == "Greedy" else "#1A73E8")
            ...
            with st.expander("Workload Balance"):
                ...
                if daily: st.bar_chart(pd.DataFrame({"Day": list(daily.keys()), "Hours": list(daily.values())}).set_index("Day"), color="#1A73E8")
            with st.expander("See Placement Reasoning"):
                for t in [x for x in res if x.get('start_time')]: st.caption(f"**{t['name']}**: {t.get('explanation', 'Fixed Event')}")
...
def render_calendar():
    ...
    if 'dismissed_overlaps' not in st.session_state:
        st.session_state.dismissed_overlaps = set()

    new_overlaps = []
    for t1, t2 in overlaps:
        pair_key = frozenset([t1['id'], t2['id']])
        if pair_key not in st.session_state.dismissed_overlaps:
            new_overlaps.append((t1, t2))

    if new_overlaps:
        overlap_descriptions = []
        for t1, t2 in new_overlaps:
            s1 = safe_parse_datetime(t1['start_time'])
            time_str = s1.strftime('%A %H:%M') if s1 else "Unknown"
            overlap_descriptions.append(f"**{t1['name']}** and **{t2['name']}** at {time_str}")

        overlap_text = " | ".join(overlap_descriptions)
        st.warning(f"**Overlap detected:** {overlap_text}")

        c1, c2 = st.columns(2)
        if c1.button(":material/check: Keep Overlaps", use_container_width=True):
            for t1, t2 in new_overlaps:
                st.session_state.dismissed_overlaps.add(frozenset([t1['id'], t2['id']]))
            st.rerun()
        if c2.button(":material/auto_fix_high: Re-solve All Conflicts", type="primary", use_container_width=True):
            resolved = resolve_overlaps(non_break_tasks)
            if resolved:
                moved_names = ", ".join([f"{r['name']} moved to {r['new_day']} {r['new_time']}" for r in resolved])
                st.toast(f"Re-solved: {moved_names}")
            st.session_state.dismissed_overlaps = set()
            st.rerun()
...
def edit_dialog(task):
    ...
    default_end_time = current_end.time() if current_end else dt_time(14, 0)

    new_date = rc1.date_input("Date", value=default_date, format="DD/MM/YYYY", key=f"rdate_{task['id']}")
    new_start_t = rc2.time_input("Start", value=default_start_time, key=f"rstart_{task['id']}")
    new_end_t = rc3.time_input("End", value=default_end_time, key=f"rend_{task['id']}")

    if st.button(":material/edit_calendar: Move to New Time", use_container_width=True):
        new_start_str = datetime.combine(new_date, new_start_t).isoformat()
        new_end_str = datetime.combine(new_date, new_end_t).isoformat()
        conn = get_connection(); c = conn.cursor()
        c.execute("UPDATE tasks SET start_time = ?, end_time = ? WHERE id = ?", (new_start_str, new_end_str, task['id']))
        conn.commit(); conn.close()
        st.session_state.dismissed_overlaps = set()
        st.rerun()
...
def render_focus_mode():
    ...
    conn = get_connection(); cursor = conn.cursor(); cursor.execute("SELECT * FROM tasks WHERE completed = 0 AND module != 'Break'"); tasks = [dict(row) for row in cursor.fetchall()]; conn.close()
    if not tasks: st.info("No tasks available to focus on. Add some tasks first."); return
    st.markdown(f"<div class='tip-box'><strong>Focus Tip:</strong> {get_focus_tip()}</div>", unsafe_allow_html=True)
    task_options = {f"{t['name']} ({t['duration']} mins)": t for t in tasks}; selected_task_name = st.selectbox("Select Task", list(task_options.keys())); selected_task = task_options[selected_task_name]
    border_color, bg_color, text_color = get_event_color(selected_task)
    st.markdown(f"<div class='focus-card'><div style='border-left: 4px solid {border_color}; padding-left: 16px; margin-bottom: 12px;'><h3 style='margin: 0; color: #0F172A;'>{selected_task['name']}</h3></div><div style='display: flex; gap: 24px; color: #64748B; font-size: 14px;'><span><strong>Module:</strong> {selected_task.get('module', 'N/A')}</span><span><strong>Priority:</strong> {selected_task.get('priority', 'Medium')}</span><span><strong>Duration:</strong> {selected_task.get('duration', 0)} mins</span></div></div>", unsafe_allow_html=True)
    if st.button(":material/play_circle: Start Session", type="primary", use_container_width=True):
        my_bar = st.progress(0, text="Locking in..."); ph = st.empty()
        for percent_complete in range(100):
            time.sleep(0.05); my_bar.progress(percent_complete + 1, text=f"Focusing: {selected_task['name']} — {percent_complete + 1}%")
            if percent_complete % 25 == 0: ph.info(f"Coach: {get_focus_tip()}")
        my_bar.progress(100, text="Session complete!"); ph.empty(); st.balloons()
        st.markdown(f"<div class='focus-card' style='border-top: 3px solid #0B8043; text-align: center;'><h3 style='color: #0B8043; margin: 0 0 8px 0;'>{get_hype_message()}</h3><p style='color: #64748B; margin: 0;'>{selected_task['name']} · {selected_task.get('duration', 0)} mins completed</p></div>", unsafe_allow_html=True)
        mark_task_completed(selected_task['id'], selected_task['module'], selected_task['duration']); time.sleep(3); st.rerun()
...
def render_stats():
    ...
    conn = get_connection(); df = pd.read_sql_query("SELECT * FROM completion_log", conn); conn.close()
    if df.empty: st.info("Complete some tasks to unlock your analytics."); return
    total_tasks = len(df); total_hours = round(df['duration'].sum() / 60, 1); current_streak = get_streak()
    ...
    with c_chart1:
        ...
        ax.set_xticks([]); ax.spines['top'].set_visible(False); ax.spines['right'].set_visible(False); ax.spines['bottom'].set_visible(False); ax.spines['left'].set_visible(False); ax.tick_params(left=False)
        st.pyplot(fig)
    with c_chart2:
        ...
        mod_hours.plot.pie(ax=ax2, autopct='%1.0f%%', colors=colors[:len(mod_hours)], textprops={'color': "#0F172A", 'weight': 'bold', 'fontsize': 9}); ax2.set_ylabel('')
        st.pyplot(fig2)
...
def render_settings():
    ...
    conn = get_connection(); cursor = conn.cursor(); cursor.execute("SELECT * FROM user_preferences"); prefs = {row['key']: row['value'] for row in cursor.fetchall()}
    ...
    st.markdown(f"<div class='settings-summary'>Your schedule runs <strong>{ds:02d}:00 – {de:02d}:00</strong>, max <strong>{mh} hours/day</strong>, <strong>{bm} min</strong> breaks. Weekends: <strong>{'Included' if inc_weekends else 'Excluded'}</strong>. Style: <strong>{study_style}</strong>.</div>", unsafe_allow_html=True)
    with st.form("settings_form"):
        st.subheader("Scheduling Defaults")
        st.markdown("<div class='form-section-label'>Working Hours</div>", unsafe_allow_html=True)
        c1, c2 = st.columns(2); day_start = c1.number_input("Day Start Hour", min_value=0, max_value=23, value=ds, help="Earliest hour for scheduling."); day_end = c2.number_input("Day End Hour", min_value=0, max_value=23, value=de, help="Latest hour for scheduling.")
        st.markdown("<div class='form-section-label'>Session Limits</div>", unsafe_allow_html=True)
        c3, c4 = st.columns(2); max_hours = c3.number_input("Max Study Hours/Day", min_value=1, max_value=12, value=mh, help="Maximum study hours per day."); break_mins = c4.number_input("Default Break (mins)", min_value=0, max_value=60, value=bm, help="Break time between sessions.")
        st.markdown("<div class='form-section-label'>Preferences</div>", unsafe_allow_html=True)
        c5, c6 = st.columns(2); include_weekends = c5.checkbox("Include Weekends", value=inc_weekends); study_style_val = c6.selectbox("Study Style", ["Balanced", "Short Bursts (30-45 min)", "Deep Work (90+ min)"], index=["Balanced", "Short Bursts (30-45 min)", "Deep Work (90+ min)"].index(study_style) if study_style in ["Balanced", "Short Bursts (30-45 min)", "Deep Work (90+ min)"] else 0)
        if st.form_submit_button("Save Preferences", type="primary", use_container_width=True):
            ...
            for k, v in new_prefs.items(): cursor.execute("INSERT OR REPLACE INTO user_preferences (key, value) VALUES (?, ?)", (k, v))
            conn.commit(); st.success("Settings saved.")
    st.markdown("---"); st.markdown("### Danger Zone"); st.caption("Permanently delete all tasks and progress. This cannot be undone.")
    if 'confirm_reset' not in st.session_state: st.session_state.confirm_reset = False
    if not st.session_state.confirm_reset:
        if st.button(":material/warning: Reset Database"): st.session_state.confirm_reset = True; st.rerun()
    else:
        ...
        if col1.button("Confirm Reset", type="primary"):
            if confirm_text == "RESET": cursor.execute("DELETE FROM tasks"); cursor.execute("DELETE FROM completion_log"); conn.commit(); st.session_state.confirm_reset = False; st.rerun()
            else: st.error("Type RESET exactly to confirm.")
